...
public class CIHPickupHalf extends ContainersClickHandler {

    /**
     * For the use of {@link InventoryAction#PICKUP_HALF}
     */
    public CIHPickupHalf() { super(InventoryAction.PICKUP_HALF); }
    ...
    public ContainersInteractionResult handleContainersOperation(@Nullable GOOPCDeployed deployed, @Nullable ContainerInventory observed, @Nullable GOOPCPlayer rpg, @NotNull Player player, @NotNull InventoryClickEvent event) {
        ...
        finalStored.setAmount(OotilityCeption.RoundToInt(Math.floor(finalStored.getAmount() * 0.5D)));
        finalPickup.setAmount(OotilityCeption.RoundToInt(Math.ceil(finalPickup.getAmount() * 0.5D)));
